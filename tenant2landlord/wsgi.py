...
def _get_message(ident: int) -> TenantMessage:
    ...
    try:
        return TenantMessage.get(
            (TenantMessage.id == ident) & (TenantMessage.customer == CUSTOMER.id)
        )
    except TenantMessage.DoesNotExist:
        raise JSONMessage("The requested message does not exist.", status=404) from None
...
def toggle_message(ident: int) -> JSONMessage:
    ...
    message.save()
    return JSONMessage("The message has been toggled.", read=message.read, status=200)
